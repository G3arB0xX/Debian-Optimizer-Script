...
# 若从 v4 升级且 /var/lib/lego 已有数据，执行一次迁移（幂等）
lego migrate --path=/var/lib/lego
```
...
chmod 700 /var/lib/lego /var/lib/lego/accounts /var/lib/lego/certificates
```
...
在 TUI 中添加域名后，脚本会自动调用 `/usr/local/bin/debopti-lego-run-once.sh` 完成首次申请。非 root 运行时会通过 `sudo` 写入 `/var/lib/lego/accounts` 与证书目录。

若系统已安装 Ferron（`debopti` 部署的 Ferron 模块），新域名默认开启 `DEBOPTI_FERRON_PUSH=true`，证书申请/续期成功后自动复制到 `/etc/ferron/certs/<主域名>.{crt,key}` 并重载 Ferron。目录权限为 `750 root:debopti-certs`，证书 `644`、私钥 `640`；`ferron` 与 Podman 的 `apps` 用户会自动加入 `debopti-certs` 组；其他本地服务将运行用户加入该组即可读取私钥（容器内需额外挂载目录并设置 `GroupAdd=keep-groups`，见 [03-podman.md §13.1](03-podman.md#131-在容器中挂载-tls-证书lego--ferron)）。可在域名详情中关闭「Ferron 同步」。

已安装 Lego 后，在 TUI 子菜单选择 **`I`（安装 / 更新）** 可升级二进制并重新部署托管脚本。

手动重试或故障排查时，可执行：

```bash
sudo /usr/local/bin/debopti-lego-run-once.sh /etc/lego/envs/example.com.env issue
```

等价的底层 `lego run` 命令（仅供调试）：

```bash
sudo /usr/local/bin/lego run \
    --env-file="/etc/lego/envs/example.com.env" \
...
TUI 添加域名或手动重试时，调用 `/usr/local/bin/debopti-lego-run-once.sh`（完整内容参见 `templates/apps/lego/debopti-lego-run-once.sh`）：
...
# 续期检测
sudo /usr/local/bin/debopti-lego-run-once.sh /etc/lego/envs/example.com.env renew

# 自动判断（无证书则申请，有证书则续期）
sudo /usr/local/bin/debopti-lego-run-once.sh /etc/lego/envs/example.com.env auto
```
...
#### 4.5.1 自动续期轮询脚本 `/usr/local/bin/debopti-lego-renew.sh`
...
if [[ -z "$PRIMARY_DOMAIN" ]]; then
    echo "❌ [Lego Hook] 无法确定主域名（需 LEGO_HOOK_CERT_NAME 或命令行参数）。" >&2
    exit 1
fi
...
if [[ "${DEBOPTI_FERRON_PUSH:-}" == "true" ]]; then
    if [[ -d "/etc/ferron" ]] || command -v ferron >/dev/null 2>&1; then
        ...
        # 权限：目录 750 root:debopti-certs，crt 644，key 640（组内服务可读私钥）
...
#### 4.5.2 推送钩子脚本 `/usr/local/bin/debopti-lego-hook.sh`
...
ReadWritePaths=/var/lib/lego /etc/ferron/certs
```
