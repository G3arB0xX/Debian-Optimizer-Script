...
本文档是 Fish + Yazi + Micro 终端工具链的**使用教程**，与 `debopti` 脚本部署结果对齐。

**前提条件**：已通过 [06-devops-tools.md](06-devops-tools.md)（安装与配置部署）或 TUI 完成工具链安装；配置文件完整内容参见 `templates/apps/devops/`。

**文档分工**：`06-devops-tools.md` 仅保留安装、配置与卸载步骤；快捷键说明、协同工作流与日常操作均在本篇。
...
本项目不部署 Micro 侧边栏文件树插件，改用 **Yazi + Micro 联动流**：

1. Fish 中输入 `y` 唤起 Yazi，进行目录导航与文件筛选。
2. Yazi 中选中文件按 `Enter`，由 Micro 前台打开编辑。
3. Micro 中 `Ctrl+Q` 退出后回到 Yazi，目录位置由 Wrapper 自动同步。
...
Yazi 支持在界面内对选中文件执行 Shell 命令，无需退出文件管理器。
...
| 按键  | 模式  | 说明                              |
| --- | --- | ------------------------------- |
| `;` | 非阻塞 | 后台异步执行，适合备份、下载等无交互任务            |
| `:` | 阻塞  | 接管终端屏幕，可查看输出或与脚本交互，完成后按键返回 Yazi |
...
| 占位符           | 含义                 | 示例                           |
| ------------- | ------------------ | ---------------------------- |
| `%s`          | 所有已选中文件的绝对路径（空格分隔） | `tar -czvf backup.tar.gz %s` |
| `%s1` / `%s2` | 第 1 / 第 2 个选中文件的路径 | `diff %s1 %s2`               |
| `%d`          | 当前工作目录的绝对路径        | `echo %d`                    |


**示例：对选中日志批量运行脚本**

1. 用 `Space` 勾选 `log1.txt`、`log2.txt`。
2. 按 `:` 打开命令行，输入：
...
- **前台阻塞**：勾选文件 → `:` → `tar -czvf logs.tar.gz %s`
- **后台非阻塞**：选中项目 → `;` → `cp -r %s /backup/%d/`
