...
编辑 `/etc/apt/sources.list`，填入以下内容（完整内容参见 `templates/system/sources.list.buster.global`；中国大陆镜像版参见 `templates/system/sources.list.buster.cn`）：
...
写入 `/etc/sysctl.d/99-debopti-optimize.conf`（完整内容参见 `templates/system/99-debopti-optimize.conf`）：
...
写入 `/etc/security/limits.d/99-debopti-nofile.conf`（完整内容参见 `templates/system/99-debopti-nofile.conf`）：
...
写入 `/etc/sysctl.d/99-debopti-forwarding.conf`（完整内容参见 `templates/system/99-debopti-forwarding.conf`）：
...
写入 `/etc/default/zramswap`（完整内容参见 `templates/system/zramswap`）：
