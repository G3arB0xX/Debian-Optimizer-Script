...
```bash
cat > /etc/sysctl.d/99-debopti-optimize.conf << 'EOF'
# 解除系统级文件句柄上限限制
fs.file-max = 1048576
...
# TCP 连接回收与复用优化 (针对频繁建连的反代与高并发场景)
net.ipv4.tcp_tw_reuse = 1               # 允许将 TIME-WAIT 状态的套接字重新用于新的 TCP 连接
net.ipv4.tcp_fin_timeout = 15           # 保持套接字在 FIN-WAIT-2 状态的时间（秒）
net.ipv4.tcp_max_tw_buckets = 50000      # 系统同时保持 TIME_WAIT 套接字的最大数量
...
# 加强防 SYN 洪水攻击能力，提高高并发连接队列上限
net.core.somaxconn = 32768              # 系统中每一个端口最大的监听队列长度
net.core.netdev_max_backlog = 32768     # 允许送到队列的数据包最大数
net.ipv4.tcp_max_syn_backlog = 32768    # 记录的那些尚未收到客户端确认信息的连接请求的最大值

# 开启 TCP Fast Open 减少握手往返（值 3 允许客户端与服务端相互发送 TFO 选项）
...
```bash
# 系统级 PAM 限制
cat > /etc/security/limits.d/99-debopti-nofile.conf << 'EOF'
* soft nofile 1048576
* hard nofile 1048576
...
# 写入 /etc/default/zramswap 配置文件
cat > /etc/default/zramswap << 'EOF'
# 压缩算法：zstd 压缩率最高，lz4 速度最快，lzo 兼容性最好
ALGO=zstd
# 使用物理内存的百分比作为 ZRAM 大小（50% 表示 1GB 内存分配 500MB ZRAM）
PERCENT=50
